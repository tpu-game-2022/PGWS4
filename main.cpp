...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	for (auto adpt : adapters)
	{
		...
		else if (strDesc.find(L"Microsoft") != std::string::npos)
		{
			tmpAdapter = adpt;
			break;
		}
...
#pragma pack(pop)
	...
	for (int i = 0; i < pmdMaterials.size(); ++i)
	{
		...
		{//スプリッタがある
			...
			else if(GetExtension(namepair.first)=="spa")
			{
				...
				texFileName = namepair.first;//他拡張子でもとにかく最初のものを入れる
	...
	descTblRange[1].BaseShaderRegister = 2; //1番スロットから
	...
	samplerDesc[1].AddressW = D3D12_TEXTURE_ADDRESS_MODE_CLAMP;//奥行
	...
	while (true)
	{
		...
		UINT cbvsrvIncSize = _dev->GetDescriptorHandleIncrementSize(
			D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) * 5;//4倍
