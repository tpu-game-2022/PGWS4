...
static float GetYFromXOnBezier(float x, const XMFLOAT2& a, const XMFLOAT2& b)
{
	...
	constexpr float epsilon = 0.0005f;  //誤差の範囲内か同化に使用する定数
	constexpr uint8_t n = 12;  //最大ループ回数

	for (int i = 0; i < n; ++i) {
		//f(t)求める
		auto ft = k0 * t * t * t + k1 * t * t * k2 * t - x;
		//もし結果が0に近い（誤差の範囲内）なら打切り
		if (ft <= epsilon && ft >= -epsilon)break;

		t -= ft / 2.0;
	}
	//すでに求めたいtは出ているのでyを計算する
	float r = 1.0f - t;
...
void PMDActor::LoadVMDFile(const char* filepath, const char* name)
{
	...
	RecursiveMattrixMultipy(_boneNodeTable["センター"], XMMatrixIdentity());
...
void PMDActor::RecursiveMattrixMultipy(BoneNode& node, const DirectX::XMMATRIX& mat)
{
	_boneMatrices[node.boneIdx] = mat;

	for (BoneNode* cnode : node.children)
	{
		RecursiveMattrixMultipy(*cnode, _boneMatrices[cnode->boneIdx] * mat);
	}
}
