...
#pragma pack(pop)
	...
	_sphResources.resize(_materialNum, nullptr);
	_spaResources.resize(_materialNum, nullptr);
	_toonResources.resize(_materialNum, nullptr);
	...
	for (int i = 0; i < pmdMaterials.size(); i++)
	{
		...
		if (sphFileName != "") {
			...
			_sphResources[i] = _dx12.GetTextureByPath(sphFileName.c_str());
		}
		if (spaFileName != "") {
			auto spaFilePath = GetTexturePathFromModelAndTexPath(strModelPath, spaFileName.c_str());
			_spaResources[i] = _dx12.GetTextureByPath(spaFileName.c_str());
		}
	}
...
void PMDActor::Draw()
{
	...
	_cmdList->SetDescriptorHeaps(1, transheaps);
...
static float GetYFromXOnBezier(float x, const XMFLOAT2& a, const XMFLOAT2& b)
{
	...
	constexpr float epsilon = 0.0005f; // 誤差の範囲内か同課に使用する定数
...
{
	DWORD elapsedTime = timeGetTime() - _startTime; // 計か時間を測る
	...
	// 行列情報クリア（していないと前フレームのポーズが重ね掛けされえてモデルが壊れる）
	std::fill(_boneMatrices.begin(), _boneMatrices.end(), XMMatrixIdentity());
	...
	for (auto& bonemotion : _motiondata)
	{
		...
		if (rit == keyframes.rend())continue; // 合致するのもがなければ飛ばす
