...
void PMDActor::MotionUpdate() {
	...
	float fframeNo = 30.0f * (float)elapsedTime / 1000.0f;
	...
	for (auto& bonemotion : _motiondata) {
		...
		auto it = rit.base();
		if (it != keyframes.end()) {
			float t = static_cast<float>(fframeNo - rit->frameNo) /
				static_cast<float>(it->frameNo - rit->frameNo);
			t = GetYFromXOnBezier(t, it->p1, it->p2);

			//rotation = XMMatrixRotationQuaternion(rit->quaternion)*(1-t)+ XMMatrixRotationQuaternion(it->quaternion)*t;
