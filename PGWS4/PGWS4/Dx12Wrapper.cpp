...
void Dx12Wrapper::CreateSceneView()
{
	...
	ThrowIfFailed(_sceneConstBuff->Map(0, nullptr, (void**)&_mapSceneData)); // マップ
...
Dx12Wrapper::Dx12Wrapper(HWND hwnd, unsigned int width, unsigned int height)
{
	...
	InitializeBackBuffers(_backBuffers, _rtvHeaps, _dev.Get(), _swapchain.Get()); // バックバッファー
	InitializeDepthBuffer(_depthBuffer, _dsvHeap, width, height, _dev.Get()); // 深度バッファー
