...
private:
	...
	Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> _rtvHeaps = nullptr; // レンダーターゲット用でスクリプタヒープ
	Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> _dsvHeap = nullptr; // 深度バッファービュー用でスクリプタヒープ
	...
	SceneData* _mapSceneData; // マップ先を示すポインター
...
private:
	...
	static Microsoft::WRL::ComPtr<ID3D12Device> InitializeDevice(IDXGIFactory6* dxgiFactry);
	static void InitializeCommand(Microsoft::WRL::ComPtr<ID3D12CommandAllocator>& cmdAllocator, Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& cmdList,
