...
#pragma pack()
	...
	{
		PMDBone& pd = pmdBones[idx];
		boneNames[idx] = pd.boneName;
		BoneNode& node = _boneNodeTable[pd.boneName];
		node.boneIdx = idx;
		node.startPos = pd.pos;
	}

	//親子関係を構築する
	for (PMDBone& pd : pmdBones)
	{
		//親インデックスをチェック（ありえない番号なら飛ばす）
		if (pmdBones.size() <= pd.parentNo)
		{
			continue;
		}
		std::string& parentName = boneNames[pd.parentNo];
		_boneNodeTable[parentName].childern.emplace_back(&_boneNodeTable[pd.boneName]);
	}
...
void PMDActor::LoadVMDFile(const char* filepath, const char* name)
{
	...
	fseek(fp, 50, SEEK_SET);//最初の50バイトは飛ばしておっけ
...
void PMDActor::RecursiveMatrixMultipy(BoneNode& node, const DirectX::XMMATRIX& mat)
{
	...
	for (BoneNode* cnode : node.childern)
	{
