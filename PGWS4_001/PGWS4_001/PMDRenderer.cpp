...
static ComPtr<ID3D12Resource> CreateTexture(ID3D12Device* dev, unsigned int val)
{
	...
	ComPtr<ID3D12Resource> coltexBuff = nullptr;
	auto result = dev->CreateCommittedResource(&texHeapProp, D3D12_HEAP_FLAG_NONE, &resDesc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, nullptr, IID_PPV_ARGS(coltexBuff.ReleaseAndGetAddressOf()));
...
static ComPtr<ID3D12Resource> CreateGrayGradationTexture(ID3D12Device* dev)
{
	...
	HRESULT result = dev->CreateCommittedResource(&texHeapProp, D3D12_HEAP_FLAG_NONE, &resDesc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, nullptr, IID_PPV_ARGS(gradBuff.ReleaseAndGetAddressOf()));
...
ComPtr<ID3D12RootSignature> PMDRenderer::CreateRootSignature(ID3D12Device* dev)
{
	...
	ThrowIfFailed(dev->CreateRootSignature(0, rootSigBlob->GetBufferPointer(), rootSigBlob->GetBufferSize(), IID_PPV_ARGS(rootsignature.ReleaseAndGetAddressOf())));
...
ComPtr<ID3DBlob> PMDRenderer::LoadShader(LPCWSTR pFileName, LPCSTR pEntrypoint, LPCSTR pTarget)
{
	...
	HRESULT result = D3DCompileFromFile(pFileName, nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, pEntrypoint, pTarget, D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, 0, &sBlob, &errorBlob);
...
ComPtr<ID3D12PipelineState> PMDRenderer::CreateBasicGraphicsPipeline(ID3D12Device* dev, ID3DBlob* vsBlob, ID3DBlob* psBlob, ID3D12RootSignature* rootsignature)
{
