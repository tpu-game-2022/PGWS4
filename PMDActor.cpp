...
void PMDActor::LoadVMDFile(const char* filepath, const char* name)
{
	...
	for (VMDKeyFrame& keyframe : keyframes)
	{
		...
		fread(&keyframe. frameNo, sizeof(keyframe.frameNo) +        //フレーム番号
			sizeof(keyframe.location) +                             //位置（IKのときに使用可能）
	...
	{
		//_motiondata[f.boneName][f.frameNo]=
			//KeyFrame(f.frameNo, XMLoadFloat4(&f.quaternion));
...
{
	DWORD elapsedTime = timeGetTime() - _startTime;//経過時間を計る
...
void PMDActor::Update()
{
	...
	//
	////左腕を90゜曲げる
	///*BoneNode& LeftArmNode = _boneNodeTable["左腕"];
	//DirectX::XMFLOAT3& LeftArmPos = LeftArmNode.startPos;
	//DirectX::XMMATRIX LeftArmMat=
	//	XMMatrixTranslation(-LeftArmPos.x, -LeftArmPos.y, -LeftArmPos.z)
	//	* XMMatrixRotationZ(XM_PIDIV2)
	//	* XMMatrixTranslation(LeftArmPos.x, LeftArmPos.y, LeftArmPos.z);*/

	////左ひじを－90゜曲げる
	//BoneNode& LeftElbowNode = _boneNodeTable["左ひじ"];
	//DirectX::XMFLOAT3& LeftElbowPos = LeftElbowNode.startPos;
	//DirectX::XMMATRIX LeftElbowMat=
	//	XMMatrixTranslation(-LeftElbowPos.x, -LeftElbowPos.y, -LeftElbowPos.z)
	//	* XMMatrixRotationZ(-XM_PIDIV2)
	//	* XMMatrixTranslation(LeftElbowPos.x, LeftElbowPos.y, LeftElbowPos.z-2);

	////右腕を90゜曲げる
	///*BoneNode& RightArmNode = _boneNodeTable["右腕"];
	//DirectX::XMFLOAT3& RightArmPos = RightArmNode.startPos;
	//DirectX::XMMATRIX RightArmMat =
	//	XMMatrixTranslation(-RightArmPos.x, -RightArmPos.y, -RightArmPos.z)
	//	* XMMatrixRotationZ(-XM_PIDIV2)
	//	* XMMatrixTranslation(RightArmPos.x, RightArmPos.y, RightArmPos.z);*/

	////右ひじを90゜曲げる
	//BoneNode& RightElbowNode = _boneNodeTable["右ひじ"];
	//DirectX::XMFLOAT3& RightElbowPos = RightElbowNode.startPos;
	//DirectX::XMMATRIX RightElbowMat =
	//	XMMatrixTranslation(-RightElbowPos.x, -RightElbowPos.y, -RightElbowPos.z)
	//	* XMMatrixRotationZ(XM_PIDIV2)
	//	* XMMatrixTranslation(RightElbowPos.x, RightElbowPos.y, RightElbowPos.z-2);


	////_boneMatrices[LeftArmNode.boneIdx] = LeftArmMat;
	//_boneMatrices[LeftElbowNode.boneIdx] = LeftElbowMat;
	////_boneMatrices[RightArmNode.boneIdx] = RightArmMat;
	//_boneMatrices[RightElbowNode.boneIdx] = RightElbowMat;
